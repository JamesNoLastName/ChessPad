...
fun MainApp() {
    ...
    var showTextRecognitionScreen by remember { mutableStateOf(false) }
    LaunchedEffect(Unit) {
        delay(3000)
    ...
    Crossfade(targetState = showSplashScreen) { isSplash ->
        if (isSplash) {
            ...
            if (showTextRecognitionScreen) {
                TextRecognitionScreen(onBack = { showTextRecognitionScreen = false })
            } else {
                ChessGamesScreen(onShowTextRecognition = { showTextRecognitionScreen = true })
            }
        }
    }
}
...
@Composable
fun ChessGamesScreen(onShowTextRecognition: () -> Unit) {
    ...
    Scaffold(
        ...
        Column(modifier = Modifier.fillMaxSize()) {
            Box(
                ...
                IconButton(
                    onClick = onShowTextRecognition,
                    modifier = Modifier.align(Alignment.CenterStart).padding(start = 16.dp)
                ) {
                    Icon(Icons.Default.Search, contentDescription = "Text Recognition", tint = Color.White)
                }
                Text(text = "ChessPad", fontSize = 24.sp, color = Color.White)
                IconButton(
...
fun ChessGameCard(game: ChessGame, onClick: () -> Unit) {
    Card(
        ...
        Column(modifier = Modifier.padding(16.dp), horizontalAlignment = Alignment.CenterHorizontally) {
            ...
            Text(text = game.date, style = MaterialTheme.typography.bodyMedium)
        }
    }
}
        ...
        text = {
            Column {
                ...
                Button(
                    ...
                    Text(text = if (gameLat != null && gameLng != null) "Change Game Location" else "Set Game Location")
                }
    ...
    if (showMap) {
        Box(
            ...
            Card(
                ...
                MapScreen(
                    selectedLocation = if (gameLat != null && gameLng != null) LatLng(gameLat!!, gameLng!!) else null,
...
fun GameDetailsPanel(game: ChessGame, onClose: () -> Unit) {
    Box(
        modifier = Modifier.fillMaxSize().background(Color.Black.copy(alpha = 0.4f)).clickable { onClose() },
        contentAlignment = Alignment.Center
    ) {
        Card(
            modifier = Modifier.fillMaxWidth(1f).fillMaxHeight(0.6f).padding(16.dp),
            ...
            Column(modifier = Modifier.padding(16.dp), horizontalAlignment = Alignment.CenterHorizontally) {
                ...
                Text(text = game.title, fontSize = 20.sp, style = MaterialTheme.typography.headlineSmall, textAlign = TextAlign.Center)
                Spacer(modifier = Modifier.height(8.dp))
                Text(text = "Date: ${game.date}", fontSize = 16.sp, textAlign = TextAlign.Center)
                Spacer(modifier = Modifier.height(16.dp))
                Text(text = "Moves:\n${game.moves}", fontSize = 14.sp, modifier = Modifier.fillMaxWidth(), textAlign = TextAlign.Left)
                if (game.latitude != null && game.longitude != null) {
                    Spacer(modifier = Modifier.height(8.dp))
                    Text(text = "Location: (${game.latitude}, ${game.longitude})", fontSize = 14.sp, textAlign = TextAlign.Center)
                }
            }
        }
    }
...
@Composable
fun TextRecognitionExample(bitmap: Bitmap) {
    var recognizedText by remember { mutableStateOf("No text recognized") }
        ...
        Button(onClick = {
            scope.launch {
                processImageForTextRecognition(
                    ...
                    onFailure = { error -> recognizedText = "Error: ${error.message}" }
                )
            }
        }) {
            Text("Recognize Text")
...
fun TextRecognitionScreen(onBack: () -> Unit) {
    ...
    val sampleBitmap: Bitmap = BitmapFactory.decodeResource(context.resources, R.drawable.sample_image)
    Scaffold(
        topBar = {
            TopAppBar(
        ...
        Box(modifier = Modifier.padding(innerPadding)) {
            TextRecognitionExample(bitmap = sampleBitmap)
        }
    }
}
